...
def test_historical_authorization_record_is_exact_target_and_nonreleasing() -> None:
...
def test_final_tag_message_is_not_draft() -> None:
    text = (ROOT / "release/v1.3.0/RC1_ANNOTATED_TAG_MESSAGE.txt").read_text(
        encoding="utf-8"
    )
...
def test_authorization_workflow_is_validation_only() -> None:
    text = (ROOT / ".github/workflows/rc1-tag-authorization.yml").read_text(
        encoding="utf-8"
    )
...
def test_historical_authorization_manifest_is_frozen() -> None:
    record = read_json(ROOT / RECORD_PATH)
    path = ROOT / "release/v1.3.0/RC1_TAG_AUTHORIZATION_MANIFEST.json"
    assert (
        sha256_file(path)
...
def test_tag_creation_script_never_recreates_existing_tag() -> None:
