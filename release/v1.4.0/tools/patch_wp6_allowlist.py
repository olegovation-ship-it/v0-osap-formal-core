...
HOSTED_CI_CORRECTIVE_VERIFIER=ROOT/'release/v1.4.0/tools/verify_wp6_hosted_ci_regression_corrective_repair.py'
...
def hosted_ci_successor_attested():
 if not HOSTED_CI_CORRECTIVE_VERIFIER.is_file(): return False
 try:
  spec=importlib.util.spec_from_file_location('wp6_hosted_ci_corrective_allowlist',HOSTED_CI_CORRECTIVE_VERIFIER)
  if spec is None or spec.loader is None: return False
  module=importlib.util.module_from_spec(spec); spec.loader.exec_module(module)
  return module.successor_overlay_attestation('auto') is not None
 except Exception:
  return False
def verify_historical_allowlist():
 run('git','cat-file','-e',BASELINE+'^{commit}')
 run('git','cat-file','-e',CANONICAL_WP6_HEAD+'^{commit}')
 if run('git','merge-base','--is-ancestor',BASELINE,CANONICAL_WP6_HEAD,check=False).returncode: raise SystemExit('WP6 canonical head does not descend from baseline')
 rows=lines('diff','--name-status','--no-renames',BASELINE,CANONICAL_WP6_HEAD)
 statuses={}
 for row in rows:
  parts=row.split('\t'); statuses[parts[-1]]=parts[0]
 exp=set(EXPECTED); changed=set(statuses)
 if changed!=exp: raise SystemExit('WP6 HISTORICAL ALLOWLIST mismatch missing='+repr(sorted(exp-changed))+' extra='+repr(sorted(changed-exp)))
 if any(p.startswith(FORBIDDEN) for p in changed): raise SystemExit('frozen predecessor path changed in canonical WP6 commit')
 if any(statuses.get(p)!='A' for p in EXPECTED): raise SystemExit('canonical WP6 paths are not all additive')
 if run('git','rev-parse','refs/tags/v1.3.0^{}').stdout.strip()!=TAG_TARGET: raise SystemExit('stable tag target changed')
 print('WP6 EXACT 54-PATH ADDITIVE ALLOWLIST: PASS (detached canonical replay; successor overlay attested)')
if __name__=='__main__':
 if hosted_ci_successor_attested():
  verify_historical_allowlist(); raise SystemExit(0)
 run('git','cat-file','-e',BASELINE+'^{commit}')
