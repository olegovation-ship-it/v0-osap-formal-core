...
EXPECTED = ['docs/gate3/cluster_b/WP5_BUILD_SPECIFICATION.md', 'release/v1.4.0/GATE3_CLUSTER_B_WP5_BUILD_PLAN.json', 'release/v1.4.0/tools/patch_wp5_build_spec_allowlist.py', 'schemas/v1.4.0/gate3_cluster_b_wp5_build_plan.schema.json', 'scripts/verify_gate3_cluster_b_wp5_build_spec.py', 'tests/test_gate3_cluster_b_wp5_build_spec.py']
...
def main() -> int:
    ...
    if run('git','merge-base','--is-ancestor',BASELINE,'HEAD',check=False).returncode:
        raise SystemExit('WP5 build-spec baseline is not an ancestor of HEAD')
    changed=set(lines('diff','--name-only',BASELINE,'--'))
    changed.update(lines('ls-files','--others','--exclude-standard'))
    if changed != set(EXPECTED):
        raise SystemExit('WP5 BUILD-SPEC ALLOWLIST mismatch missing='+repr(sorted(set(EXPECTED)-changed))+' extra='+repr(sorted(changed-set(EXPECTED))))
    statuses={}
