...
EVIDENCE_CLOSURE_RECORD = RELEASE / "RC1_RELEASE_EVIDENCE_CLOSURE_RECORD.json"
...
if EVIDENCE_CLOSURE_RECORD.is_file():
    ...
    assert closure["release_actions"]["rc1_tag_created"] is True
    assert closure["release_actions"]["github_prerelease_created"] is True
    assert closure["release_actions"]["final_tag_created"] is False
    assert closure["release_actions"]["zenodo_version_created"] is False
    audit_status = "RC1_GATE_AUDIT_PRESERVED_AFTER_PRERELEASE"
else:
    for body in [readme, status]:
        assert (
    ...
    audit_status = "RC1_AUDIT_READY_CI_PENDING"
...
evidence = {
    ...
    "historical_preservation": {
        ...
        "gate_audit_created_release": False,
...
print(
    "PASS: RC1 gate audit executable restored; 36 theorem records verified and "
    f"{len(killed)}/{len(killed)} negative gate mutants killed. "
    "Historical RC1 publication evidence does not authorize a final release."
)
